...
def compute_adaptive_weights(resume_skills: set, jd_skills: set) -> dict:
    ...
    OR if the JD produced zero detected skills specifically (a strong signal
    the taxonomy doesn't cover this domain, since real JDs almost always
    name concrete skills), shifts the 'skills' weight into 'semantic' so
    under-covered fields aren't unfairly penalized -- or falsely rewarded --
    just because our dictionary is incomplete for them.
    """
    weights = dict(BASE_WEIGHTS)
    total_detected = len(resume_skills) + len(jd_skills)

    taxonomy_likely_mismatched = (len(jd_skills) == 0) or (total_detected < LOW_SKILL_SIGNAL_THRESHOLD)

    if taxonomy_likely_mismatched:
...
def compute_ats_score(resume_text: str, job_description_text: str, field: str = "Technology / ML") -> dict:
    ...
    if jd_skills:
        ...
        # No skills detected in the JD at all. This is either a genuinely
        # skill-less JD (rare), or -- much more likely in practice -- our
        # taxonomy doesn't cover this domain/field (a wrong field selection,
        # or a niche field with no matching entries). We cannot honestly
        # claim "perfect skill match" (1.0) in either case, since we have
        # no real evidence either way. Use a neutral 0.5 rather than a
        # falsely charitable 1.0 -- combined with compute_adaptive_weights()
        # shrinking this component's influence on the final score anyway.
        skills_score = 0.5
